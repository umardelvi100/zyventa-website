...
export function AddToCartButton({ product, quantity = 1, compact = false, disabled = false }: Props) {
  ...
  return (
    <motion.button
      onClick={handleAdd}
      whileTap={{ scale: 0.97 }}
      className="flex w-full items-center justify-center gap-2 rounded-md bg-accent px-6 py-3.5 font-semibold text-accent-foreground transition hover:opacity-90"
    >
      {justAdded ? (
        <motion.span
          initial={{ scale: 0 }}
